// 순수 계산 모듈(입출력 없음) — rpg-combat.js와 같은 패턴. 영지 경제(식량/골드)와 영지배치 용병
// 개인효과(훈련소/병원/사기진작소)를 "영지일" 단위로 정산한다. 영지일은 실시간이 아니라 플레이어가
// 턴포인트를 "그 레벨의 턴 상한만큼" 소모할 때마다 하루씩 지나가는 것으로 정의됨(레벨이 오르면 턴 상한도
// 오르므로 하루가 점점 길어져 상대적으로 여유가 생기는 구조) - api/_rpg/adventure.js가 매 모험마다
// 이 결과를 트랜잭션 안에서 반영.
import { turnCapForLevel } from './api/_rpgTurns.js';
import {
  TERRITORY_JOBS, FOOD_PER_DAY_PER_FARMER, FOOD_CONSUMPTION_PER_DAY_PER_WORKER,
  GOLD_PER_MISSING_FOOD,
...
// 훈련소=경험치(본인 레벨 초과 불가), 병원=체력회복+부상치료 가속, 사기진작소=멘탈저항 영구 소량 상승(상한 있음)
...
export function settleTerritoryDays(character, daysToProcess, playerLevel = character.level || 1) {
  ...
  // 4) 개간지 골드 산출(인원수 기준, 시설레벨 보너스 적용) - 용병 상주 급여는 없음(고용비는 선술집에서
  // 고용할 때 한 번만 냄)
  ...
  // 훈련소/병원/사기진작소 배치 용병 개인 효과 - 다른 일자리(개간지/농장/방벽/연공실/연무장)는 아직
  // 개인 효과 없음(applyMercTerritoryJobEffect 참고), 해당 없는 용병은 그대로 통과
  ...
  return {
    ...
    goldIncome, foodEmergencyCost, goldDelta,

...
export function defaultCharacter(slot, now = Date.now()) {
  const base = {
    ...
    facilityDays: { clearing: 0, training: 0, ramparts: 0, farm: 0, morale: 0 }, // 시설별 누적 배치 영지일(레벨업 재료)
    facilityLevels: { clearing: 0, training: 0, ramparts: 0, farm: 0, morale: 0 }, // facilityDays로부터 계산된 현재 레벨(전역 보너스 % 산정용, data/rpg/facilities.js 참고)

...
- **정적 UI 문구 진행 상황**: `page-rpg.js`의 `// ── ...` 주석으로 구분된 28개 화면 섹션 중 **완료: 캐릭터선택/직업선택/상태바/탭, 모험탭+성+지역미리보기, 로어알림, 퀘스트행, 의사NPC, 영지탭(휴식/회복)**. 부상/붕대 관련 문구(치료 토스트, 부상요약 등 여러 섹션에 걸친 것)도 이 김에 같이 정리함. `BODY_PART_NAMES` 상수(모듈로드시 고정되는 버그 패턴)를 `bodyPartName(part)` 함수로 교체(`t()` 호출이라 매번 최신 lang 반영) — 앞으로 이런 소규모 상수 맵도 발견하면 같은 방식으로 함수화할 것.
  - 남은 섹션(아직 하나도 안 건드림, `grep -n "^// ──" page-rpg.js`로 항상 최신 줄번호 확인): 직업교관/대장간/선술집/마을탭/상점탭/마켓탭/창고탭/게시판/인벤토리탭/파티섹션/영지현황판/영지탭(용병관리)/부상요약(일부는 위에서 같이 처리함, 남은 부분 확인 필요)/캐릭터탭/장비추천/장비창/부직업선택/탐험일지/포션자동사용

...
- **"Me's" 영어 문법 이슈 해결 완료 (2026-08-08 무렵).** `buildCombatant`(rpg-combat.js)가 자기 자신 라벨을 문법적 위치별로 3종 생성하도록 확장: `label`(주어, "I"/이름), `labelObject`(목적어, "me"/이름), `labelPossessive`(소유격 전체구, "My"/"이름's"). 로케일 키도 `rpg.ui.combat.self`(단일) → `rpg.ui.combat.selfSubject`/`selfObject`/`selfPossessive`(3종)로 분리(ko.js는 전부 "나"로 동일, en.js만 I/me/My로 실제 분기). `rpg.log.*` 템플릿 중 `{actor}'s`가 하드코딩됐던 곳은 전부 `{actorPoss}`로, `{target}`이 목적어/소유격으로 쓰이던 곳(holyLeech/monsterHit/heal/monsterSkillHit/bloodDrainTick의 목적어, indomitableWill/dreadAura/lowHpWarning/injuryWorsened의 소유격)은 `targetObj`/`targetPoss`로 en.js에서만 분리해 교체(ko.js 템플릿은 조사가 위치 무관이라 안 건드림). 60판 시뮬레이션으로 "Me's"/"My's" 패턴 0건 확인.
  - **위 flavor 전치사 이슈도 해결 완료 (같은 세션 이어서 처리, 2026-08-08).** `monsterHitIntro.*`에 "at"/"on" 전치사 추가("struck hard"→"struck hard at", "bit down"→"bit down on", "pounced"→"pounced on", "smashed down"→"smashed down on", "slashed"는 원래 타동사라 그대로), `monsterCritIntro.*`는 "of X" 어색함을 피하려 아예 재작성("landed a devastating blow to the weak point"→"landed a devastating critical blow on", "completely broke through the defense"→"completely overwhelmed"). `monsterMiss.*`는 두 템플릿(monsterAttackMiss: `{miss} {monster}'s attack!`, monsterSkillMiss: `{miss} it!`)에서 공유되는데, 기존엔 "dodged it"처럼 목적어를 이미 포함해서 monsterAttackMiss에 쓰면 "dodged it Wolf Pup's attack!"로 중복됐음 → flavor에서 내장 목적어를 빼고("barely dodged", "rolled out of the way of") monsterSkillMiss 템플릿 쪽에 `it!`을 추가해서 양쪽 다 자연스럽게 맞춤. 80판 시뮬레이션으로 이상한 조합 없음 확인.
- **인도네시아어(`locales/id.js`) 번역 완료 (2026-08-08).** `rpg.*` 키 1359개 전부 번역 — ko.js와 정확히 1:1 매칭 검증 완료(키셋 diff 없음, 중복 없음). 전투로그 키는 en.js와 동일한 플레이스홀더 세트(`{actorPoss}`/`{targetObj}`/`{targetPoss}` 등)를 그대로 씀. `selfSubject`/`selfObject`/`selfPossessive` 3종은 인도네시아어에 격변화가 없어서 전부 "saya"로 동일(ko.js가 전부 "나"인 것과 같은 이유).
  - **작업 중 발견해서 같이 고친 버그**: `buildCombatant`의 `labelPossessive`가 self가 아닐 때 무조건 `` `${name}'s` `` 형태로 영어식 아포스트로피-s를 하드코딩하고 있었음 — en.js만 쓰던 동안은 문제없었지만, id.js도 `{actorPoss}`/`{targetPoss}`를 그대로 쓰게 되면서 "Prajurit Sewaan's Tekad..." 처럼 인도네시아어 문장에 영어 소유격 표기가 섞여 나오는 버그로 이어짐. `lang === 'en'`일 때만 `'s`를 붙이고 그 외 언어는 이름을 그대로 쓰도록 수정(rpg-combat.js). 추가로 에이전트가 번역한 id.js 자체에서도 두 가지를 발견해 수정: (1) `monsterAttackMiss`/`monsterSkillMiss` 템플릿이 "berhasil {miss}"처럼 "성공" 접두어를 붙였는데 `monsterMiss` 플레이버 0/3번 값 자체가 이미 "berhasil"로 시작해서 "berhasil berhasil ditangkis" 같은 중복이 생김 → 템플릿에서 접두어 제거. (2) `dangerRetreat`/`moraleSwapResisted`/`moralePushedSwap`가 "-nya"(3인칭 소유격 접미사)를 템플릿에 하드코딩해서 대상이 본인(saya)일 때 "nyawanya"(그의/그녀의 목숨) 같은 3인칭 오류가 남 → "-nya" 없이도 자연스러운 문장으로 재구성. 80판 시뮬레이션(id/en 둘 다)으로 위 4가지 문제 전부 재발 없음 확인.
- **다음 세션이 할 일**: 나머지 15개 언어(en/ko/id 제외) 아직 미착수.
